...
namespace GameOfLife
{
    internal class Window : GLWindow
    {
        ...
        protected override void OnUpdateFrame(FrameArgs e)
        {
            ...
            var dThread = GetWindowThreadProcessId(desktop, out _);
            var ptr = GetForegroundWindow();
            var wThread = GetWindowThreadProcessId(desktop, out _);

            if (ptr != IntPtr.Zero && wThread != dThread && ptr != WindowInfo.Handle && ptr != desktop)
            {
                var rect = new Rect();
